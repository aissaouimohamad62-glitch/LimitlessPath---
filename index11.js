...
// ==========================================
// ==========================================
// MOTIVATIONAL QUOTES SYSTEM (AR, EN, KO)
// ==========================================
const QuoteSystem = {
    quotes: [
        {
    ...
    updateQuote() {
        ...
        // التصحيح هنا: استخدام this.quotes بدلاً من SystemQuotes غير الموجودة
...
// تفعيل النظام تلقائياً عند تحميل الصفحة
window.addEventListener('DOMContentLoaded', () => {
    QuoteSystem.init();
});

// ==========================================
// FATIGUE TIME ENGINE (REALISTIC DAILY STRESS)
// ==========================================
const FatigueEngine = {
    init() {
        this.update();
        // تحديث الإرهاق كل دقيقة تلقائياً
    ...
    update() {
        ...
        if (currentTotalMinutes >= endMinutes || currentTotalMinutes < startMinutes) {
            // بعد العاشرة ليلاً أو قبل الخامسة فجراً، الإرهاق يصل لحده الأقصى إذا لم يتم النوم
            fatiguePercent = 100;
        } else {
            // حساب النسبة المئوية بناءً على الوقت المنقضي
            fatiguePercent = ((currentTotalMinutes - startMinutes) / (endMinutes - startMinutes)) * 100;
        }
        ...
        if (fatigueBar && fatigueText) {
            ...
            fatigueText.textContent = `${Math.floor(fatiguePercent)}%`;
            
            // تغيير لون شريط الإرهاق إذا تجاوز 80% للتنبيه
            if (fatiguePercent > 80) {
                fatigueBar.className = "absolute inset-y-0 left-0 bg-gradient-to-r from-red-900 to-red-600 shadow-[0_0_10px_rgba(220,38,38,0.4)] transition-all duration-700";
            } else {
                fatigueBar.className = "absolute inset-y-0 left-0 bg-gradient-to-r from-yellow-900 to-yellow-600 shadow-[0_0_10px_rgba(202,138,4,0.4)] transition-all duration-700";
            }
        }
    }
};
